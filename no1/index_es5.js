...
function mouseEvent3($pad1, $pad2, $pad1_array, $pad2_array, displayMode, $pad1Class) {
    $pad1.mouseover(function (e) {
        for (var i = 0; i < $pad2_array.length; i++) {
...
function appear($item) {
    ...
    return $item.offset().top + 60 < $window.scrollTop() + $window.height() && //元素底部到页面顶部距离-60  大于  浏览器顶部到页面顶部距离
...
function city_select() {
    ...
    if ($search_city_byletter != undefined) {
        ...
        for (var i = 0; i < $search_city_byletter.length; i++) {
            _loop(i);
        }
    }
    //选择城市
    if ($hotcity_list != undefined) {
        ...
        for (var i = 0; i < $hotcity_list.length; i++) {
            _loop2(i);
        }
    }
    if ($hd_city != undefined) {
        ...
        for (var i = 0; i < $hd_city.length; i++) {
            _loop3(i);
        }
    }
}
        ...
        if (!inanime) {
            ...
            $($promo_pics[hidenum]).fadeOut(50, function () {
                $($promo_pics[shownum]).fadeIn(50, function () {
                    for (var i = 0; i < $promo_btn.length; i++) {
...
function shortcut() {
    ...
    for (var j = 0; j < $exclusive_ul_li.length; j++) {
        _loop4(j);
    }
    //触发新面板
    ...
    for (var i = 0; i < 3; i++) {
        ...
        $($extend_content_close[i]).click(function () {
            ...
            for (var _i = 0; _i < $extend_content.length; _i++) {
                $($extend_content[_i]).css("display", "none");
                $($extend_title[_i]).attr('class', "");
            }
        });
    }
}
        ...
        $($pa_lis[i]).mouseenter(function () {
            if (!inanime) {
                ...
                $($pa_lis_img[i]).animate({ width: wValue,
                    ...
                    top: "-" + 0.2 * $($pa_lis_img[i]).height() / 2 }, 200, function () {
                    inanime = false;
                });
            }
        });
...
function index_roof() {
    ...
    for (var i = 0; i < $small_pic_img.length; i++) {
        _loop7(i);
    }
}
...
function index_floor() {
    ...
    function flash($index_floor_imgs) {
        $index_floor_imgs.each(function (index) {
            $(this).mouseenter(function () {
                $(this).animate({ opacity: 0.6 }, 50).animate({ opacity: 1 }, 50);
            });
        });
    }
}
